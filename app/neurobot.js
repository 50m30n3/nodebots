var debug = require( 'debug' )( 'nodebots:neurobot' );
...
function neurobot( x, y, rot, parent )
{
	...
	if( ! parent )
	{
		...
		this.braindepth = 4+Math.floor(Math.random()*7);
		this.brainsize = 4+Math.floor(Math.random()*4)*4;
	...
	else
	{
		...
		{
			this.mutationrate = parent.mutationrate+((Math.random()*2.0-0.485)*parent.mutationrate)*0.1;
	...
	this.step = function( bots, food )
	{
		...
		if( this.x > conf.width ) this.x -= conf.width;
		if( this.x < 0 ) this.x += conf.width;
		if( this.y > conf.height ) this.y -= conf.height;
		if( this.y < 0 ) this.y += conf.height;
	...
	this.getdata = function()
	{
		...
		return data.toUpperCase() + '!http://erikentrich.de/#seekers';
	}
